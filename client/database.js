...
exports.select = (table, clause, skip, limit) => query(
  `SELECT * FROM ${table} ${clause} LIMIT ${skip}, ${limit}`, 
  {}, 
  identity => identity
)

exports.insert = (table, item) => query(
  `INSERT INTO ${table} SET ?`, 
  item, 
  async _ => item
)

exports.update = (table, item) => query(
  `UPDATE ${table} SET ? WHERE _id = ${connection.escape(item._id)}`, 
  item, 
  async _ => item
)

exports.deleteOne = (table, itemId) => query(
  `DELETE FROM ${table} WHERE _id = ${connection.escape(itemId)}`, 
  {}, 
  result => result.affectedRows
)

exports.count = table => query(
  `SELECT COUNT(*) FROM ${table}`, {}, result => result[0]['COUNT(*)']
)

exports.describeDatabase = () => query(
  'SHOW TABLES', 
  {}, 
  async result => {
...
const describeTable = table => query(
  `DESCRIBE ${table}`, 
  {}, 
  result => {
...
const query = (query, values, handler) => new Promise((resolve, reject) => {
